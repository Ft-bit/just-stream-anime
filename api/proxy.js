// api/proxy.js — CORS + Referer proxy for HLS segments
module.exports = async function handler(req, res) {
  ...
  const ALLOWED = ['uwucdn.top','owocdn.top','vault-','kwik','pahe','animepahe',
                   'megaplay','vidnest','akamaized.net','cloudfront.net','.m3u8','.ts','.mp4','.m4s'];
  if (!ALLOWED.some(s => url.includes(s))) return res.status(403).send('domain not allowed');
