...
async function resolveViaScraper(title, epNum) {
  try {
    ...
    // API returns { m3u8: "...", referer: "https://kwik.si/", proxy_url: "..." }
    const url     = m3u8D.m3u8 || m3u8D.url || m3u8D.source
                    || (typeof m3u8D === 'string' ? m3u8D : null);
    const referer = m3u8D.referer || null;   // ← critical: CDN needs this
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Strategy 1: AnimePahe chain via Vercel API
    if (title) {
      ...
      if (result?.url) {
        return res.status(200).json({
          ...
          url:      result.url,
          referer:  result.referer,   // ← returned to frontend
    ...
    // Strategy 3: Fallback → client embeds the player
    return res.status(200).json({
      success:    false,
      title:      title || 'Anime',
      episode:    epNum,
